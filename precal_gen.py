...
def main():
    ...
    if type == .3:  # 3 linear
        ...
        anstex = "The answer is $\displaystyle\\frac{%d}{%dx+%d}+\displaystyle\\frac{%d}{%dx+%d}+\displaystyle\\frac{%d}{%dx+%d}$" % a.rcoeff[
                                                                                                                                     0:9]
    ...
    if type == 5:
        ...
        a.calcangle()
        # print a.angle
        problem = 'Calculate the angle $\\theta$ (in degrees) between the vectors $' + str(a.vec1) + '$ and $' + str(
            a.vec2) + '$.'
        problem = problem.replace('[', '\langle ')
        problem = problem.replace(']', '\\rangle ')
    ...
    if type == 6:
        ...
        a = vector.vector(3)
        a.makeplane()
        problem = "Find the equation of the plane through the points $" + str(a.point) + '$, $' + str(
            a.point2) + '$, and $' + str(a.point3) + '$. <br> (Answer in the form $ax+ by+cz +d=0$.)'
        problem = problem.replace('[', '(')
        problem = problem.replace(']', ')')
        ans = a.plane
