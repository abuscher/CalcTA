...
def reduce(original):
    ...
    for i in range(1):
        ...
        for j in range(len(s)):
            if j < len(s) - 1:
                if s[j] == '.' and is_int(s[j + 1]):
                    trigger = False
                elif is_int(s[j]) and s[j + 1] == '.':
                    trigger = False
            if is_int(s[j]):
                if j == 0:
                    if is_int(s[0]) and is_int(s[1]) == 0 and trigger == True:
                        new += s[0] + '.'
                    else:
                        new += s[j]
                elif j == len(s) - 1:
                    if is_int(s[j]) and is_int(s[j - 1]) == 0 and s[j - 1] != '.' and trigger == True:
                        new += s[j] + '.'
                    else:
                        new += s[j]
                else:
                    if is_int(s[j - 1]) == 0 and is_int(s[j + 1]) == 0 and s[j - 1] != '.' and trigger == True:
                        new += s[j] + '.'
                    else:
                        new += s[j]
            else:
...
class integral(object):
    def __init__(self, type):
        ...
        if type == 'double':
            ...
            # Word Problem
            bank[15] = [
                'Find the volume below the the surface $z=x^2+y^2$ and the above $z=0$ on $x=[-1,1]$ and $y=[-1,2]$.',
                '8', '']
            bank[16] = [
                'Find the volume below the the surface $z=\sin(x)\cdot\sin(y) + 2$ and the above $z=0$ on $x=[0,\pi]$ and $y=[0,\pi]$.',
                '2*pi^2+4', '']
            # two more
            ...
            bank[8] = ['Find the area of the region bounded by the curve $r^2=9\sin(3\\theta)$ on $\\theta=[0,\pi/3]$.',
                       '3', '']
            bank[9] = [
                'Evaluate the integral $\displaystyle\iint\limits_A{e^{x^2+y^2}\ dA}$ where $A$ is the origin-centered unit circle.',
                '(e-1)*2*pi', '']
            bank[10] = [
                'Evaluate the integral $\displaystyle\iint\limits_A{\sin(x^2+y^2)\ dA}$ where $A$ is the region bounded above by $y=\sqrt{4-x^2}$ and below by $y=x$ and $y=-x$.',
                'pi/2*sin(2)^2', '']
            bank[11] = [
                'Evaluate the integral $\displaystyle\iint\limits_A{x^2+y^2+1\ dA}$ where $A$ is the origin-centered unit circle.',
                '3/2*pi', '']
            bank[12] = [
                'Evaluate the integral $\displaystyle\iint\limits_A{\\frac{1}{x^2}\ dA}$ where $A$ is the region $1\leq x^2+y^2\leq 4$ and $0\leq\\theta\leq\pi/4$.',
                'ln(2)', '']
            bank[13] = [
                'Evaluate the integral $\displaystyle\iint\limits_A{xy\ dA}$ where $A$ is the region $x^2+y^2\leq 4$ and $0\leq\\theta\leq\pi/2$.',
                '2', '']
            ...
            bank[5] = ['Find the volume of the region in the first quadrant bounded by the plane $2x+y+7z-6=0$', '36/7',
                       '']
            bank[6] = [
                'Find the volume of the region bounded by the planes $z=0$, $z=1-y$, $z=1+y$, $x=2$, and $x=-3$.', '5',
                '']
            bank[7] = ['Find the volume of the region above $z=0$ and below $z=e^{x+y}$ over the region $x,y=[-1,1]$.',
                       'e^2+e^(-2)-2', '']
        ...
        # p=rho, ph=phi, t=theta,
            ...
            bank[3] = ['', ('-\sqrt{1- y^2 - z^2}', '\sqrt{1- y^2 - z^2}'), ('\sqrt{1-z^2}', '\sqrt{1- z^2}'),
                       ('-1', '1'), 'xyz', '4*pi/3']
        ...
        if (type == 'double' or type == 'polar') and ptype == 'bound':
            ...
            bounds = bank[pick][3] + bank[pick][2] + bank[pick][1] + (
            toTEX(bank[pick][0]), bank[pick][4][0], bank[pick][4][1], bank[pick][4][2])
            bounds = bank[pick][3] + bank[pick][2] + bank[pick][1] + (
            toTEX(bank[pick][0]), bank[pick][4][0], bank[pick][4][1], bank[pick][4][2])
...
def main():
    ...
    if type1 == 1:
        ...
        if subtype >= 3:
            ...
            func = 'f(x,y,z)'
            df = '(' + f[0] + ')' + '*' + implicit.dtuv[0][uorv] + '+' + '(' + f[1] + ')' + '*' + implicit.dtuv[1][
                uorv] + '+' + f[2] + '*' + implicit.dtuv[2][uorv]
            df = df.replace('x', implicit.tuv[0])
            df = df.replace('y', implicit.tuv[1])
            df = df.replace('z', implicit.tuv[2])
        if subtype == 1: probtex = 'Given the function $' + func + '=' + toTEX(
            implicit.f) + '$, <br>with $x(u,v)=' + toTEX(implicit.tuv[0]) + '$ and $y(u,v)=' + toTEX(
            implicit.tuv[1]) + '$ find $' + der + '$.'
        if subtype == 2: probtex = 'Given the function $' + func + '=' + toTEX(
            implicit.f) + '$, <br>with $x(t)=' + toTEX(implicit.tuv[0]) + '$ and $y(t)=' + toTEX(
            implicit.tuv[1]) + '$ find $' + der + '$.'
        if subtype == 3: probtex = 'Given the function $' + func + '=' + toTEX(
            implicit.f) + '$, <br>with $x(u,v)=' + toTEX(implicit.tuv[0]) + '$, $y(u,v)=' + toTEX(
            implicit.tuv[1]) + '$ and $z(u,v)=' + toTEX(implicit.tuv[2]) + '$ find $' + der + '$.'
        if subtype == 4: probtex = 'Given the function $' + func + '=' + toTEX(
            implicit.f) + '$, <br>with $x(t)=' + toTEX(implicit.tuv[0]) + '$, $y(t)=' + toTEX(
            implicit.tuv[1]) + '$ and $z(t)=' + toTEX(implicit.tuv[2]) + '$ find $' + der + '$.'
    ...
    if type1 == 2:
        ...
        if subtype == 1:
            ...
            probtex = 'Given the function $' + func + '=' + toTEX(
                grad.f) + '$, find the derivative at $(%d,%d)$ in the direction $ \left\langle %d,%d \\right\\rangle $.' % tuple(
        ...
        if subtype == 2:
            ...
            func = 'f(x,y,z)'
            probtex = 'Given the function $' + func + '=' + toTEX(
                grad.f) + '$, find the derivative at $(%d,%d,%d)$ in the direction $\left\langle %d,%d,%d \\right\\rangle$.' % tuple(
                vect.point + vect.vec1)
        roundlim = 4
    ...
    if type1 == 3:
        ...
        bank['Find the maximum volume of a box with a surface area of $24$.'] = ['8', '$V=$',
        ...
        bank[
            ...
            '4', '$V=$', 'Optimize $f=xyz$ constrained by $g=2xy+2yz+xz-12=0$.']
        # more geo? cylinder?
        bank['Find the maximum volume of a cylinder inscribed in a sphere with radius $r_s=5$.'] = ['32*pi', '$V=$',
                                                                                                    'Use the constraint $r_s=\sqrt{r^2+(h/2)^2}$.']
        bank['Find the maximum volume of a rectangular prism inscribed in a sphere with a diameter $d_s=3$.'] = ['1',
                                                                                                                 '$V=$',
                                                                                                                 'Use the constraint $d_s=\sqrt{x^2+y^2+z^2}$.']
        ...
        bank['Find the minimum distance from the surface $xyz=1$ to the origin'] = ['3^(1/2)', '',
                                                                                    'Let $f=x^2+y^2+z^2$']
        ...
        # to finish (2 each)
        bank['Find the maximum value of $x+y+z$ if the points lie on the sphere $x^2+y^2+z^2=5$.'] = ['ans', 'prefix',
                                                                                                      'hint', 'anstex']
    ...
    if type1 == 9:
        ...
        bank[
            ...
            '1', '', '']
        bank[
            'Evaluate the integral $\int_C{\\frac{y}{x}\cdot \\frac{1}{x^2} \ ds}$ where $C$ is the clockwise path from $\\theta=0$ to $\\theta=\pi/3$ on the unit circle.'] = [
            '1', '',
        ...
        # more complex
        bank['Evaluate the integral $\int_C{ds}$ where $C$ is the path from $(0,0)$ to $(1,1)$ on $y=x^4$.'] = [
            '(17^(3/2)-1)/144', '', '']
        """
        ...
        bank[
            ...
            '2*sqrt(2)*pi^2', '', 'You may need to use the identity $\sin^2(x)+\cos^2(x)=1$.']
        bank[
            ...
            '2*sqrt(2)*pi^2', '', 'You may need to use the identity $\sin^2(x)+\cos^2(x)=1$.']
        bank[
            'Evaluate the integral $\int_C{xy+z \ ds}$ where $C$ is the path along the helix  $\\vec{r}(t)=\langle\cos(t), \sin(t), t/2 \\rangle$ for $t=[0,4\pi]$.'] = [
            '2*sqrt(5)*pi^2', '',
            'You may need to use the identity $\sin^2(x)+\cos^2(x)=1$ and you will need integration by parts.']
        bank[
            'Evaluate the integral $\int_C{x^2 \ ds}$ where $C$ is the path along the helix  $\\vec{r}(t)=\langle\cos(t), \sin(t), 2t \\rangle$ for $t=[0,2\pi]$.'] = [
            'pi*sqrt(5)', '',
