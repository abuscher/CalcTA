...
class divcurl(object):
    def __init__(self):
        ...
        bank['ln(x+y)', 'e^(x*y*z)', 'sin(x*y)'] = ['1/(x+y)+x*z*e^(x*y*z)',
                                                    ['(x*cos(x*y)-x*y*e^(x*y*z))', '-y*cos(x*y)',
                                                     '(y*z*e^(x*y*z)-1/(x + y))'], '']
        ...
        bank['e^(x*y*z)', 'x*y', 'e^(x+2*z)'] = ['x+2*e^(x+2*z)+y*z*e^(x*y*z)',
                                                 ['0', 'x*y*e^(x*y*z)-e^(x+2*z)', 'y-x*z*e^(x*y*z)'], '']
        bank['x^y', 'y^z', 'z^x'] = ['x*z^(x-1)+x^(y-1)*y+y^(z-1)*z', ['(-y^z*ln(y))', '(-z^x*ln(z))', '(-x^y*ln(x))'],
                                     '']
        bank['(x+y+z)', 'z+y*(cos(x^2))^2', 'tan(x)^2'] = ['cos(x^2)^2 + 1', ['-1', '1 - 2*tan(x)*(tan(x)^2 + 1)',
                                                                              '- 4*x*y*cos(x^2)*sin(x^2) - 1'], '']
        bank['(x*z+y)^5', '(y+z)^x', 'y/(x*z)'] = ['x*(y+z)^(x-1)+5*z*(y+x*z)^4-y/(x*z^2)',
                                                   ['1/(x*z) - x*(y + z)^(x - 1)', '5*x*(y + x*z)^4 + y/(x^2*z)',
                                                    'ln(y+z)*(y + z)^x-5*(y + x*z)^4'], '']
        bank['e^(x*y*z)', 'sin(x*y*z)', 'ln(x*y*z)'] = ['y*z*e^(x*y*z)+ x*z*cos(x*y*z)+1/z',
                                                        ['1/y-x*y*cos(x*y*z)', 'x*y*e^(x*y*z) - 1/x',
                                                         'y*z*cos(x*y*z) - x*z*e^(x*y*z)'], '']
        bank['(x+y^2)^5', 'sin(3*x+y)', '4+z'] = ['5*(y^2 + x)^4 +cos(3*x + y)+ 1',
                                                  ['0', '0', '3*cos(3*x+y)-10*y*(y^2 + x)^4'], '']
        bank['y+e^(x*y)', 'e^(x*y)-z', 'e^(z)'] = ['e^(z)+x*e^(x*y)+y*e^(x*y)', ['1', '0', 'y*e^(x*y)-x*e^(x*y)-1'], '']
        bank['ln(x*y)', 'ln(x+2*y+3*z)', '1/(x+y)'] = ['1/x + 2/(x + 2*y + 3*z)',
                                                       ['- 1/(x + y)^2 - 3/(x + 2*y + 3*z)', '1/(x + y)^2',
                                                        '1/(x + 2*y + 3*z) - 1/y'], '']
        # bank['','','']=['',[],'']
...
class partial(object):
    def __init__(self, numvar, ptype):
        ...
        if numvar == 2 and (ptype == 'trans' or ptype == 'mixed'):
            ...
            bank['x^y+y^x'] = ['y*x^(y-1)+y^x*ln(y)', 'x*y^(x-1)+x^y*ln(x)',
                               '-(y*x^(y-1)+y^x*ln(y))/(x*y^(x-1)+x^y*ln(x))']
        ...
        if numvar == 2 and (ptype == 'trig' or ptype == 'mixed'):
            ...
            bank['sin(x)^2*y+cos(x*y)'] = ['2*y*cos(x)*sin(x)-y*sin(x*y)', 'sin(x)^2-x*sin(x*y)',
                                           '-(y*sin(2*x)-y*sin(x*y))/(sin(x)^2-x*sin(x*y))']
            bank['cos(x)*cos(x+y)'] = ['-cos(x+y)*sin(x)-sin(x + y)*cos(x)', '-sin(x + y)*cos(x)',
                                       '- cot(x + y)/cot(x) - 1']
        ...
        if numvar == 2 and ptype == 'hypinv':
            ...
            bank['cosh(x)*sinh(y^2)'] = ['sinh(y^2)*sinh(x)', '2*y*cosh(y^2)*cosh(x)',
                                         '-(sinh(y^2)*sinh(x))/(2*y*cosh(y^2)*cosh(x))']
            bank['sinh(x^2-y^2)+y*cosh(x)'] = ['2*x*cosh(x^2-y^2)+y*sinh(x)', 'cosh(x)-2*y*cosh(x^2 - y^2)',
                                               '(2*x*cosh(x^2-y^2)+y*sinh(x))/(2*y*cosh(x^2-y^2)-cosh(x))']
        ...
        if numvar == 2 and ptype == 'mixed':
            ...
            bank['cos(x*y)*e^x'] = ['e^x*(cos(x*y)-y*sin(x*y))', '-x*e^x*sin(x*y)',
                                    '(cos(x*y)-y*sin(x*y))/(x*sin(x*y))']
            ...
            bank['x^2*sin(x*y)'] = ['2*x*sin(x*y)+x^2*y*cos(x*y)', 'x^3*cos(x*y)',
                                    '-(2*x*sin(x*y)+x^2*y*cos(x*y))/(x^3*cos(x*y))']
            bank['cos(x^2*y)*sin(x)'] = ['cos(x^2*y)*cos(x)-2*x*y*sin(x^2*y)*sin(x)', '-x^2*sin(x^2*y)*sin(x)',
                                         '(cos(x^2*y)*cos(x)-2*x*y*sin(x^2*y)*sin(x))/(x^2*sin(x^2*y)*sin(x))']
        ...
        if numvar == 3 and ptype != 'short':
            ...
            bank['(x^2 + y^2 + z^2)^(1/2)'] = ['(x)/(x^2 + y^2 + z^2)^(1/2)', '(y)/(x^2 + y^2 + z^2)^(1/2)',
                                               '(z)/(x^2 + y^2 + z^2)^(1/2)']
